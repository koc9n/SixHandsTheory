...
import com.gmail.mironchik.kos.service.SearchService;
...
public class SixHandsWayController {
    ...
    public
    @ResponseBody
    User getUser(@PathVariable String id, HttpSession session) {
        session.invalidate();
    ...
    public
    @ResponseBody
    Map getChain(@PathVariable Integer step, @PathVariable Integer id, HttpSession session) {
    ...
    private Map findMutualFriendAndStep(Integer firstId, Integer secondId) {
        ...
        while (searchService.getMutualFriends(firstFriends, secondFriends) == null) {
            ++step;
            switch (step % 2) {
                case 1:
        ...
        Map res = new HashMap();
        res.put("step", step);
        res.put("friend", searchService.getMutualFriends(firstFriends, secondFriends).iterator().next());
//        res.put("startStep",1);
...
//        res.put("second", secondId);
    ...
    public
    @ResponseBody
    Map findchain(@PathVariable Integer firstId, @PathVariable Integer secondId) {
    ...
    public
    @ResponseBody
    Map getFriends(@RequestBody FriendsRequest request, HttpSession session) throws InterruptedException {
